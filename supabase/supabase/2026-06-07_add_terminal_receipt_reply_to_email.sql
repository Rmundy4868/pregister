-- Adds terminal-backed receipt reply-to email parameter so receipt email metadata
-- can be configured per terminal/location like other operating parameters.

begin;

alter table if exists public.terminal_transaction_parameters
  add column if not exists receipt_reply_to_email text not null default '';

update public.terminal_transaction_parameters
set receipt_reply_to_email = coalesce(trim(receipt_reply_to_email), '');
