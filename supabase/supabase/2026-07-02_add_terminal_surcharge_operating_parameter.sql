-- Adds terminal-scoped operating parameter to control whether the app
-- requests processor surcharge calculation.

begin;

alter table if exists public.terminal_transaction_parameters
  add column if not exists enable_processor_surcharge boolean not null default false;

update public.terminal_transaction_parameters
set enable_processor_surcharge = coalesce(enable_processor_surcharge, false);
...
begin
  ...
  if v_row is null then
    return jsonb_build_object(
      ...
      'enable_processor_surcharge', false,
  ...
  return jsonb_build_object(
    ...
    'receipt_reply_to_email', coalesce(trim(v_row.receipt_reply_to_email), ''),
    'enable_processor_surcharge', coalesce(v_row.enable_processor_surcharge, false),
...
create function public.set_terminal_transaction_parameters_from_app(
  ...
  p_enable_processor_surcharge boolean default false,
...
begin
  ...
  insert into public.terminal_transaction_parameters (
    ...
    coalesce(trim(p_receipt_reply_to_email), ''),
    coalesce(p_enable_processor_surcharge, false),
  ...
  do update set
    ...
    enable_processor_surcharge = excluded.enable_processor_surcharge,
  ...
  return jsonb_build_object(
    ...
    'enable_processor_surcharge', coalesce(v_out.enable_processor_surcharge, false),
...
grant execute on function public.set_terminal_transaction_parameters_from_app(text, uuid, text, text, boolean, boolean, boolean, integer, boolean, integer, boolean, integer, text, boolean, jsonb)
  to anon, authenticated;
