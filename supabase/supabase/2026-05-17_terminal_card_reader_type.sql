...
alter table public.terminals
  add column if not exists card_reader_type text;

update public.terminals
set card_reader_type = 'dejavoo_p12'
where coalesce(trim(card_reader_type), '') = '';

alter table public.terminals
  alter column card_reader_type set default 'dejavoo_p12';
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'terminals_card_reader_type_check'
  ) then
    alter table public.terminals
      add constraint terminals_card_reader_type_check
      check (card_reader_type in ('none', 'dejavoo_p12'));
  end if;
end;
$$;
...
revoke all on function public.upsert_terminal_from_app(text, uuid, uuid, text, text, text, text, boolean) from public;
grant execute on function public.upsert_terminal_from_app(text, uuid, uuid, text, text, text, text, boolean) to anon, authenticated;
