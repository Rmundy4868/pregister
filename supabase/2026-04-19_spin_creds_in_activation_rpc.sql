-- Migration: Return spin_tpn and spin_auth_key from activation and list RPCs
-- so the Flutter app can persist payment credentials at activation time
-- without requiring a ?tk= URL token.

-- ─── activate_install_license ────────────────────────────────────────────────
-- Add spin_tpn and spin_auth_key to the return type.
-- Must drop old signature first because the return type is changing.
...
begin
  ...
  select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'locations' and column_name = 'created_at') into has_location_created_at;
  select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'locations' and column_name = 'name') into has_location_name;
  ...
  select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'organization_number') into has_terminal_org_number;
  ...
  select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'registered_device_id') into has_terminal_registered_device_id;
  select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'registered_device_label') into has_terminal_registered_device_label;
  select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'registered_at') into has_terminal_registered_at;
  select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'last_seen_at') into has_terminal_last_seen_at;
  select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'spin_tpn') into has_terminal_spin_tpn;
  select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'spin_auth_key') into has_terminal_spin_auth_key;
  ...
  if v_device_id is not null and has_terminal_registered_device_id then
    ...
    execute v_sql into v_terminal_id, v_location_id, v_terminal_name, v_terminal_is_active, v_spin_tpn, v_spin_auth_key using v_org_id, v_device_id;
    ...
    if v_terminal_id is not null and not coalesce(v_terminal_is_active, true) then
      ...
      if has_terminal_is_active then
        update public.terminals as t set is_active = true where t.id = v_terminal_id;
      end if;
  ...
  if v_terminal_id is null then
    ...
    if v_location_id is null then
      if has_location_name then
        insert into public.locations (organization_id, name) values (v_org_id, 'Primary Location') returning id into v_location_id;
      else
        insert into public.locations (organization_id) values (v_org_id) returning id into v_location_id;
      end if;
    end if;
    ...
    into
      v_location_display_name, v_location_terminal_licenses, v_location_terminals_active,
      v_legacy_gateway_api_login_id, v_legacy_gateway_transaction_key
    from public.locations l where l.id = v_location_id limit 1;

    select count(*)::integer into v_location_terminals_active
    from public.terminals t
    where t.location_id = v_location_id and coalesce(t.is_active, true) = true;
    ...
    v_sql := v_sql || ' limit 1';
    execute v_sql into v_terminal_id, v_terminal_name, v_terminal_is_active, v_spin_tpn, v_spin_auth_key using v_org_id, v_location_id, v_terminal_number;

    if v_terminal_id is not null and not coalesce(v_terminal_is_active, true) then
      if not coalesce(p_allow_register, false) then
        raise exception 'Terminal % is inactive', v_terminal_number;
      end if;
      if v_location_terminal_licenses > 0 and v_location_terminals_active >= v_location_terminal_licenses then
        raise exception 'Terminal license limit reached for this location';
      end if;
      if has_terminal_is_active then
        update public.terminals as t set is_active = true where t.id = v_terminal_id;
    ...
    if v_terminal_id is null then
      ...
      if has_terminal_org_number then v_sql := v_sql || ', organization_number'; end if;
      if has_terminal_number then v_sql := v_sql || ', terminal_number'; end if;
      if has_terminal_name then v_sql := v_sql || ', name'; end if;
      if has_terminal_code then v_sql := v_sql || ', code'; end if;
      if has_terminal_name_field then v_sql := v_sql || ', terminal_name'; end if;
      if has_terminal_is_active then v_sql := v_sql || ', is_active'; end if;
      v_sql := v_sql || ') values (' ||
        quote_literal(v_org_id::text) || '::uuid, ' ||
        quote_literal(v_location_id::text) || '::uuid';
      if has_terminal_org_number then v_sql := v_sql || ', ' || quote_literal(v_org_number); end if;
      if has_terminal_number then v_sql := v_sql || ', ' || quote_literal(v_terminal_number); end if;
      if has_terminal_name then v_sql := v_sql || ', ' || quote_literal('Terminal ' || v_terminal_number); end if;
      if has_terminal_code then v_sql := v_sql || ', ' || quote_literal(v_terminal_number); end if;
      if has_terminal_name_field then v_sql := v_sql || ', ' || quote_literal('Terminal ' || v_terminal_number); end if;
      if has_terminal_is_active then v_sql := v_sql || ', true'; end if;
      ...
      -- New terminal has no payment credentials yet
      v_spin_tpn := '';
      v_spin_auth_key := '';
    end if;
  end if;
  ...
  if has_terminal_registered_device_id and v_device_id is not null then
    ...
    set registered_device_id = null, registered_device_label = null
    ...
    update public.terminals as t set last_seen_at = timezone('utc', now()) where t.id = v_terminal_id;
  end if;

  if exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'application_license_number') then
    update public.terminals as t set application_license_number = v_license_key where t.id = v_terminal_id;
  elsif exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'license_number') then
    update public.terminals as t set license_number = v_license_key where t.id = v_terminal_id;
  elsif exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'terminals' and column_name = 'license_key') then
  ...
  update public.locations l set terminals_active = v_location_terminals_active where l.id = v_location_id;

  -- Re-read spin credentials in case they were set after terminal insert
