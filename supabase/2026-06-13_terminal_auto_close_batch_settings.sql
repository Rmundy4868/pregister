-- Move auto-close batch settings to terminals.
...
alter table public.terminals
  ...
  add column if not exists auto_close_batch_time text;

-- Replace the terminal upsert RPC with a terminal auto-close aware signature.
...
create or replace function public.upsert_terminal_from_app(
  ...
  p_spin_tpn text default null,
  p_spin_auth_key text default null,
  p_card_reader_type text default 'dejavoo_p12',
  p_card_reader_hpp_auth_token text default null,
  p_is_active boolean default true,
  p_receipt_printer_name text default null,
  p_auto_close_batch_enabled boolean default null,
...
declare
  ...
  v_receipt_printer_name text;
  v_auto_close_batch_enabled boolean;
  v_auto_close_batch_time text;
...
begin
  ...
  v_spin_tpn := nullif(trim(coalesce(p_spin_tpn, '')), '');
  v_spin_auth_key := nullif(trim(coalesce(p_spin_auth_key, '')), '');
  ...
  v_receipt_printer_name := nullif(trim(coalesce(p_receipt_printer_name, '')), '');
  v_auto_close_batch_enabled := coalesce(p_auto_close_batch_enabled, false);
  v_auto_close_batch_time := nullif(trim(coalesce(p_auto_close_batch_time, '')), '');
  ...
  if v_auto_close_batch_enabled and (v_auto_close_batch_time is null or v_auto_close_batch_time !~ '^\d{1,2}:\d{2}(?::\d{2})?$') then
    raise exception 'Auto-close time must be HH:mm or HH:mm:ss in 24-hour format';
  end if;
  ...
  if v_terminal_id is null then
    ...
    values (
      ...
      v_receipt_printer_name,
      v_auto_close_batch_enabled,
      v_auto_close_batch_time,
    ...
    set location_id = v_location_id,
        ...
        spin_tpn = coalesce(v_spin_tpn, spin_tpn),
        spin_auth_key = coalesce(v_spin_auth_key, spin_auth_key),
        ...
        receipt_printer_name = coalesce(v_receipt_printer_name, receipt_printer_name),
        auto_close_batch_enabled = coalesce(p_auto_close_batch_enabled, auto_close_batch_enabled),
        auto_close_batch_time = case
          when p_auto_close_batch_time is not null then v_auto_close_batch_time
          when coalesce(p_auto_close_batch_enabled, auto_close_batch_enabled) then coalesce(auto_close_batch_time, v_auto_close_batch_time)
          else auto_close_batch_time
        end,
...
revoke all on function public.upsert_terminal_from_app(
  text,
  uuid,
  uuid,
  text,
...
grant execute on function public.upsert_terminal_from_app(
  text,
  uuid,
  uuid,
  text,
