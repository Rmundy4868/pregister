-- Activation RPC: add terminal_name + master PIN + default staff creation.
-- Adds card_reader_type, card_reader_hpp_auth_token, license_key,
-- default_staff_id, default_staff_name to the return set.
-- Safe to run multiple times.

begin;

drop function if exists public.activate_install_license(text, text, text, text, text, boolean, text, text);
...
create or replace function public.activate_install_license(
  ...
  p_terminal_name       text    default null,   -- display name for the terminal
  p_master_pin          text    default null    -- creates/updates default owner staff
)
returns table (
...
declare
  ...
  v_effective_terminal_name       text;
  v_default_staff_id              uuid;
  v_default_staff_name            text := '';
  ...
  has_terminal_card_reader_type           boolean;
  has_terminal_card_reader_hpp_auth_token boolean;
...
begin
  ...
  select exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'terminals' and column_name = 'card_reader_type'
  ) into has_terminal_card_reader_type;

  select exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'terminals' and column_name = 'card_reader_hpp_auth_token'
  ) into has_terminal_card_reader_hpp_auth_token;

  -- -----------------------------------------------------------------------
  -- Build dynamic SELECT expression for terminal name column
  -- -----------------------------------------------------------------------
  ...
  -- Effective name used when creating/updating the terminal record
  v_effective_terminal_name := coalesce(nullif(trim(p_terminal_name), ''), 'Terminal ' || v_terminal_number);

  -- -----------------------------------------------------------------------
  -- Find terminal by device_id (fastest path — exact device match)
  -- -----------------------------------------------------------------------
  if v_device_id is not null and has_terminal_registered_device_id then
    ...
    -- Guard: a device already tied to one location cannot be activated to
    -- a different location without an explicit deactivate/re-activate flow.
    if v_terminal_id is not null and v_location_name is not null and has_location_name then
      ...
      execute v_sql into v_requested_location_id using v_org_id, v_location_name;

      if v_requested_location_id is not null and v_requested_location_id <> v_location_id then
        select coalesce(l.name, '')
        into v_current_location_name
        from public.locations l
        where l.id = v_location_id
        limit 1;

        raise exception
          'This device is already activated at location "%". Deactivate it there before activating at "%".',
          coalesce(nullif(v_current_location_name, ''), v_location_id::text),
          v_location_name;
      end if;
    end if;

    -- Update terminal name if a new one was provided during re-activation
    if v_terminal_id is not null and trim(coalesce(p_terminal_name, '')) != '' then
      if has_terminal_name_field then
        execute 'update public.terminals t set terminal_name = $1 where t.id = $2'
          using v_effective_terminal_name, v_terminal_id;
      elsif has_terminal_name then
        execute 'update public.terminals t set name = $1 where t.id = $2'
          using v_effective_terminal_name, v_terminal_id;
      end if;
      v_terminal_name := v_effective_terminal_name;
    end if;
  end if;

  -- -----------------------------------------------------------------------
  -- If no device match, resolve by terminal_number (or create new)
  -- -----------------------------------------------------------------------
  if v_terminal_id is null then
    ...
    -- Find existing terminal by number
    v_sql :=
    ...
    -- Create new terminal if none found
    if v_terminal_id is null then
      if not coalesce(p_allow_register, false) then
      ...
      -- Build INSERT using effective terminal name from p_terminal_name (or default)
      ...
      if has_terminal_name then
        -- Use effective terminal name instead of generic 'Terminal NNNN'
        v_sql := v_sql || ', ' || quote_literal(v_effective_terminal_name);
      end if;
      if has_terminal_code then
        v_sql := v_sql || ', ' || quote_literal(v_terminal_number);
      end if;
  ...
  -- -----------------------------------------------------------------------
  -- Update device registration on the resolved terminal
  -- -----------------------------------------------------------------------
  ...
  -- Store license key on the terminal record
  if exists (
    select 1 from information_schema.columns
  ...
  -- Recount active terminals and sync to location
  select count(*)::integer
  into v_location_terminals_active
  from public.terminals t
  where t.location_id = v_location_id
  ...
  -- -----------------------------------------------------------------------
  -- Read payment credentials from resolved terminal
  -- -----------------------------------------------------------------------
  ...
  if has_terminal_card_reader_type or has_terminal_card_reader_hpp_auth_token then
    execute
      'select ' ||
      case when has_terminal_card_reader_type then 'coalesce(t.card_reader_type, '''')' else '''''::text' end || ', ' ||
      case when has_terminal_card_reader_hpp_auth_token then 'coalesce(t.card_reader_hpp_auth_token, '''')' else '''''::text' end ||
      ' from public.terminals t where t.id = $1'
    into v_card_reader_type, v_card_reader_hpp_auth_token
    using v_terminal_id;
  end if;

  -- -----------------------------------------------------------------------
  -- Read final location display name
  -- -----------------------------------------------------------------------
  ...
  -- -----------------------------------------------------------------------
  -- Upsert default owner staff if master PIN provided
  -- -----------------------------------------------------------------------
  if p_master_pin is not null then
    v_normalized_pin := regexp_replace(trim(p_master_pin), '[^0-9]', '', 'g');

    if v_normalized_pin != '' and v_normalized_pin ~ '^[0-9]{1,6}$'
       and v_org_id is not null and v_location_id is not null then

      -- Look for existing owner staff at this org + location
      select s.id
      into v_default_staff_id
      from public.staff s
      ...
      if v_default_staff_id is null then
        -- Create the default owner staff member
        ...
        returning id into v_default_staff_id;
      else
        -- Update the PIN on the existing owner staff
        update public.staff s
        set pin = v_normalized_pin
        where s.id = v_default_staff_id;
      end if;

      -- Resolve display name for the return value
      select coalesce(s.first_name, '') ||
             case when coalesce(s.last_name, '') != '' then ' ' || s.last_name else '' end
      into v_default_staff_name
      from public.staff s
      where s.id = v_default_staff_id;
  ...
  select
    ...
    coalesce(v_card_reader_type, ''),
    coalesce(v_card_reader_hpp_auth_token, ''),
    coalesce(v_license_key, ''),
    v_default_staff_id,
    coalesce(v_default_staff_name, '');
end;
$$;

revoke all on function public.activate_install_license(text, text, text, text, text, boolean, text, text) from public;
grant execute on function public.activate_install_license(text, text, text, text, text, boolean, text, text) to anon, authenticated;
