...
class VRegisterScreen extends StatefulWidget {
  const VRegisterScreen({super.key, this.startupContextOverride});

  final Map<String, String>? startupContextOverride;

  @override
  State<VRegisterScreen> createState() => _VRegisterScreenState();
}
...
class _VRegisterScreenState extends State<VRegisterScreen> {
  ...
  final bool _showKeypadCursor = true;
  ...
  Future<Uint8List> _buildVoidReceiptPdfBytes(
    ...
    final pdf = pw.Document();
    final now = DateTime.now();
    final amount = ((detail['amount'] as num?) ?? 0).toDouble().abs();
  ...
  Future<Uint8List> _buildOpenBatchReportPdfBytes(PdfPageFormat format) async {
    ...
    final openBatchRows = _openBatchCards
        .where(
          (row) =>
    ...
    for (final row in openBatchRows) {
      final isApproved =
          (row['status']?.toString().toLowerCase() ?? '') == 'approved';
      if (!isApproved || !isTipAdjustmentRow(row)) {
        continue;
      }
      final originalId = row['original_detail_id']?.toString().trim() ?? '';
    ...
    for (final row in openBatchRows) {
      ...
      if (subtype == 's') {
        ...
        final baseAmount = (mutable['amount'] is num)
            ? (mutable['amount'] as num).toDouble()
            : 0.0;
        mutable['display_amount'] = baseAmount + tipAdd;
    ...
    final originalTotal = approvedRows
        .where((r) => (r['subtype']?.toString().toLowerCase() ?? '') == 's')
        .fold<double>(
          0,
          (sum, r) =>
              sum +
              ((r['amount'] is num) ? (r['amount'] as num).toDouble() : 0),
        );
    final tipTotal = tipAdjustByOriginalId.values.fold<double>(
      0,
      (sum, tip) => sum + tip,
    );
    final finalTotal = originalTotal + tipTotal;
  ...
  Future<void> _applyVRegisterDesktopWindowMode() async {
    ...
    for (var attempt = 0; attempt < maxAttempts; attempt++) {
      final alreadyMaximized = await windowManager.isMaximized();
      if (alreadyMaximized) {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      unawaited(_applyVRegisterDesktopWindowMode());
    });
                    ...
                    child: Column(
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            border: Border.all(color: const Color(0x422E353D)),
                            borderRadius: BorderRadius.circular(8),
                          ),
                          child: Text(
            ...
            children: [
              Positioned.fill(
                child: BackdropFilter(
                  ...
                  child: Container(color: const Color(0x422E353D)),
                ),
              ),
              Center(
                child: LayoutBuilder(
  ...
  Future<void> _voidCard(
    ...
    if (!skipConfirmation) {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text(
            'Void \$${(detail['amount'] as num).toStringAsFixed(2)} '
    ...
    if (result.success) {
      ...
      try {
        ...
        final voidedAmount =
            (detail['amount'] as num?)?.toDouble().abs() ?? 0.0;
  ...
  Future<void> _refundClosedCard(Map<String, dynamic> detail) async {
    final originalAmount = (detail['amount'] as num?)?.toDouble().abs() ?? 0;
    ...
    final cardType = detail['card_type']?.toString() ?? '';
    final cardLast4 = detail['card_last4']?.toString() ?? '';
    final gatewayToken = detail['gateway_token']?.toString() ?? '';
            ...
            Center(
              child: ConstrainedBox(
                ...
                child: AlertDialog(
                  ...
                  title: Row(
                    children: [
                      Icon(Icons.reply_rounded, color: _themeOrange, size: 16),
            ...
            .where(
              (row) =>
                  (row['batch_status']?.toString().trim().toLowerCase() ??
                          '') ==
                      'o' &&
                  (row['subtype']?.toString() ?? '') != 'v' &&
                  (row['status']?.toString() ?? '') != 'voided',
            )
            .toList();
        safeSetDialogState(setDialogState, () {
          rows = loaded;
        ...
        builder: (_) => Stack(
          ...
          children: [
            Positioned.fill(
              child: BackdropFilter(
                filter: ImageFilter.blur(sigmaX: 8, sigmaY: 8),
                child: Container(color: const Color(0x612E353D)),
              ),
            ),
            Center(
              child: ConstrainedBox(
        ...
        final filteredRows = q.isEmpty
            ...
            : rows.where((row) {
                ...
                final referenceId = (row['reference_id'] ?? row['id'])
                    ...
                    .toLowerCase();
                final haystack =
                    '$amount $authCode $last4 $dateKey $timeKey $invoiceReference $referenceId';
  ...
  Future<void> _finalizeApprovedCardSale({
    ...
    setState(() {
      ...
      final processorLines = <String>[
        'Amount: \$${cardAmount.toStringAsFixed(2)}',
        if (feeAmount > 0) 'Fee: \$${feeAmount.toStringAsFixed(2)}',
        ...
        final filteredRows = q.isEmpty
            ...
            : rows.where((row) {
                ...
                final invoiceReference =
                    ...
                    '';
                final haystack =
                    '$amount $authCode $last4 $dateKey $timeKey $invoiceReference';
                return haystack.contains(q);
              }).toList();
  ...
  Future<void> _showOpenBatchFunctionsDialog() async {
    ...
    Future<void> runAdjustGratuity(
      ...
      if (gatewayProvider == 'paaayit_online') {
        if (!mounted) return;
        await _showCopyableOperationErrorDialog(
          title: 'Tip Adjustment Not Allowed',
          message:
              'Tip adjustment is disabled for PaaayIT e-commerce payments.',
        );
        return;
      }
    ...
    final dialogWidget = StatefulBuilder(
      builder: (context, setDialogState) {
        ...
        final originalTotal = approvedRows
            .where(
              (row) => (row['subtype']?.toString().toLowerCase() ?? '') == 's',
            )
            .fold<double>(
              0,
              (sum, row) =>
    ...
    switch (label) {
      case 'Customer':
        return const _QuickActionVisual(
          iconColor: Color(0xFF0F4A92),
          backgroundColor: Color(0xFFD5E9FB),
        );
      case 'PaaayIT NOW':
        return const _QuickActionVisual(
          iconColor: Color(0xFF0A3F8E),
          backgroundColor: Color(0xFFFFF4DE),
        );
      case 'View Open Batch':
        return const _QuickActionVisual(
          iconColor: Color(0xFF005549),
          backgroundColor: Color(0xFFCFEDE8),
        );
      case 'Voids & Refunds':
        return const _QuickActionVisual(
          iconColor: Color(0xFFCF5700),
          backgroundColor: Color(0xFFFFE8CC),
        );
      case 'Clear Sale':
        return const _QuickActionVisual(
          iconColor: Color(0xFF95114B),
          backgroundColor: Color(0xFFF9D5E2),
        );
      case 'Batch Functions':
        return const _QuickActionVisual(
          iconColor: Color(0xFF31434C),
          backgroundColor: Color(0xFFDDE5EA),
        );
      case 'Close Batch':
        return const _QuickActionVisual(
          iconColor: Color(0xFF1F6A29),
          backgroundColor: Color(0xFFD8EFDB),
        );
      case 'Reports & Processes':
        return const _QuickActionVisual(
          iconColor: Color(0xFF3E1076),
          backgroundColor: Color(0xFFE2D8F2),
        );
      default:
        return const _QuickActionVisual(
  ...
  double _quickActionFontSizeForLabel(String label) {
    if (label.length > 16) return 9.8;
    if (label.length > 12) return 10.2;
    return 10.8;
  }
  ...
  Widget _buildTerminalSaleContent({
    ...
    final subtotal = _items.fold<double>(0, (sum, item) => sum + item.abs());
    final tax = _itemTaxes.fold<double>(0, (sum, item) => sum + item.abs());
    final total = subtotal + tax;

    Widget buildQuickButtons({required int crossAxisCount}) {
          ...
          return Material(
            ...
            child: Ink(
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: visual.iconColor.withValues(alpha: 0.12),
                    blurRadius: 7,
              ...
              child: InkWell(
                ...
                child: Padding(
                  ...
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(button.icon, size: 19, color: visual.iconColor),
                      const SizedBox(height: 4),
                      Text(
                        button.label,
                        maxLines: 2,
                        ...
                        textAlign: TextAlign.center,
                        style: TextStyle(
                          fontSize: _quickActionFontSizeForLabel(button.label),
                          fontWeight: FontWeight.w400,
                          height: 1.05,
                          color: visual.iconColor.withValues(alpha: 0.95),
                        ),
                      ),
                    ],
                  ),
    ...
    Widget customerPanel() {
        ...
        return Padding(
          ...
          child: Material(
            ...
            shape: RoundedRectangleBorder(
              ...
              side: BorderSide(color: cs.outlineVariant, width: 0.9),
            ),
            child: Theme(
      ...
      Widget placeholderLine(String label, String helper) {
        return Padding(
          padding: const EdgeInsets.only(bottom: 6),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 2),
            Text(
              'Customer, invoice, tax, Level II/III, line items, recurring',
              style: TextStyle(fontSize: 10.5, color: cs.onSurfaceVariant),
            ),
            const SizedBox(height: 10),
            Expanded(
                  ...
                  children: [
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            '${infoValue(_locationName, 'Not set')} · ${infoValue(_terminalName, 'Not set')}',
                            style: const TextStyle(
                              fontSize: 11.5,
                              fontWeight: FontWeight.w700,
                            ),
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 2),
                                Text(
                                  _invoiceReference.trim().isEmpty
                                      ? 'Not set yet'
                                      : _invoiceReference.trim(),
                                  style: TextStyle(
                                    fontSize: 11,
                                    color: cs.onSurfaceVariant,
                                  ),
                    ...
                    sectionShell(
                      ...
                      child: Column(
                        ...
                        children: [
                          placeholderLine(
                            'PO Number',
                            'Commercial purchasing card support',
                          ),
                          placeholderLine(
                            'Purchase Identifier',
                            'Direct marketing / rental / custom purchase IDs',
                          ),
                          placeholderLine(
                            'Merchant Tax ID',
                            'Government or corporate interchange support',
                          ),
                          placeholderLine(
                            'Ship From / Destination ZIP',
                            'Required for shipped-goods qualification',
                          ),
                          placeholderLine(
                            'Commodity / Tax Summary',
                            'Summary commodity code, tax flags, freight, duty',
                          ),
                        ],
                      ),
                    ),
                    sectionShell(
                      title: 'Line Items / Enrichment',
                      icon: Icons.inventory_2_outlined,
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(
                            'Current line item count: ${_items.length}',
                            style: const TextStyle(
                              fontSize: 11.5,
                              fontWeight: FontWeight.w700,
                            ),
                          ...
                          placeholderLine(
                            ...
                            'Needed for Level III and future Retail item master',
                          ),
                          placeholderLine(
                            'Quantity / Unit Cost / Unit of Measure',
                            'Required item math for Level III',
                          ),
                          placeholderLine(
                            'Line Tax / Discount / Ext Amount',
                            'Supports commercial card data and full retail receipts',
                          ),
                        ],
                      ),
                    ),
                    sectionShell(
                      title: 'Recurring / Tokenization',
                      ...
                      child: Column(
                        ...
                        children: [
                          placeholderLine(
                            'Save Payment Token',
                            'Reuse approved card data for future sales and recurring plans',
                          ),
                          placeholderLine(
                            'Recurring Schedule',
                            'Weekly, monthly, quarterly, annual billing plans',
                          ),
                          placeholderLine(
                            'Customer Linkage',
                            'Attach tokens and plans to customer database records',
                          ),
                        ],
                      ),
                    ),
    ...
    Widget keypadPanel() {
      return Container(
        ...
        child: Column(
          children: [
            buildQuickButtons(crossAxisCount: 4),
            const SizedBox(height: 10),
            Container(
              width: double.infinity,
                ...
                children: [
                  ...
                  if (_focusNode.hasFocus) ...[
                    ...
                    Opacity(
                      ...
                      child: Container(width: 2, height: 32, color: cs.primary),
                    ),
                  ],
                ],
              ),
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                label: Text(_terminalWarmingUp ? 'Initializing...' : 'Card'),
              ),
            ),
            ...
            Expanded(
              child: _paymentHistory.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        final entry =
                            _paymentHistory[_paymentHistory.length - 1 - index];
                        return Container(
                          padding: const EdgeInsets.symmetric(
                            horizontal: 8,
                            vertical: 6,
                    ...
                    builder: (context, constraints) {
                      if (constraints.maxWidth < 1120) {
                        return SingleChildScrollView(
                          child: Column(
                            children: [
                              SizedBox(height: 310, child: customerPanel()),
                              const SizedBox(height: 10),
                              SizedBox(height: 470, child: keypadPanel()),
                              const SizedBox(height: 10),
                              SizedBox(height: 340, child: receiptPanel()),
                            ],
                          ),
                        );
                      }
                      ...
                      return Row(
                        children: [
                          Expanded(flex: 34, child: customerPanel()),
                          const SizedBox(width: 10),
                          Expanded(flex: 32, child: keypadPanel()),
                          const SizedBox(width: 10),
                          Expanded(flex: 34, child: receiptPanel()),
                        ],
                      );
                    },
                  ),
  ...
  Widget _buildLeftMenu() {
    return AnimatedContainer(
      ...
      curve: Curves.easeInOut,
      width: _isLeftMenuExpanded ? 230 : 68,
            ...
            child: _isLeftMenuExpanded
                ? SizedBox(
                    ...
                    child: FilledButton.icon(
                      ...
                      onPressed: () => unawaited(_returnToLoginScreen()),
                      icon: const Icon(Icons.close),
  ...
  Widget build(BuildContext context) {
    const mutedPrimaryTextColor = Color(0xFF3A424B);
    const mutedSecondaryTextColor = Color(0xFF677483);
    final cs = Theme.of(context).colorScheme.copyWith(
      onSurface: mutedPrimaryTextColor,
      onSurfaceVariant: mutedSecondaryTextColor,
    );
                              ...
                              child: IgnorePointer(
                                child: ImageFiltered(
                                  ...
                                  child: Stack(
                                    children: const [
                                      Align(
                                        alignment: Alignment(-0.96, -0.92),
                                        child: _AmbientBubble(
                                          size: 170,
                                          color: Color(0x66FF7A92),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.98, -0.45),
                                        child: _AmbientBubble(
                                          size: 120,
                                          color: Color(0x62FFCD6A),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.97, 0.02),
                                        child: _AmbientBubble(
                                          size: 135,
                                          color: Color(0x6056D67F),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.95, 0.52),
                                        child: _AmbientBubble(
                                          size: 145,
                                          color: Color(0x6048B2FF),
                                        ),
                                      ),
                                      Align(
                                        ...
                                        child: _AmbientBubble(
                                          size: 165,
                                          color: Color(0x639D7BFF),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.96, -0.90),
                                        child: _AmbientBubble(
                                          size: 170,
                                          color: Color(0x65FFE06B),
                                        ),
                                      ),
                                      Align(
                                        ...
                                        child: _AmbientBubble(
                                          size: 120,
                                          color: Color(0x62FF8C98),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.97, 0.04),
                                        child: _AmbientBubble(
                                          size: 135,
                                          color: Color(0x605ACD84),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.95, 0.50),
                                        child: _AmbientBubble(
                                          size: 145,
                                          color: Color(0x614AAAFF),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.92, 0.90),
                                        child: _AmbientBubble(
                                          size: 165,
                                          color: Color(0x649B74FF),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.55, -0.98),
                                        child: _AmbientBubble(
                                          size: 140,
                                          color: Color(0x61FF6E8F),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.12, -0.99),
                                        child: _AmbientBubble(
                                          size: 125,
                                          color: Color(0x5EFFBC60),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.34, -0.98),
                                        child: _AmbientBubble(
                                          size: 140,
                                          color: Color(0x60A86EFF),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.52, 0.98),
                                        child: _AmbientBubble(
                                          size: 150,
                                          color: Color(0x6056C98A),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.08, 0.99),
                                        child: _AmbientBubble(
                                          size: 130,
                                          color: Color(0x5E5CAEFF),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.38, 0.98),
                                        child: _AmbientBubble(
                                          size: 150,
                                          color: Color(0x61FF9F74),
                                        ),
                                      ),
                                      Align(
                                        ...
                                        child: _AmbientBubble(
                                          size: 250,
                                          color: Color(0x88FF5E7A),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.32, 0.08),
                                        child: _AmbientBubble(
                                          size: 220,
                                          color: Color(0x84FFB156),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.32, 0.08),
                                        child: _AmbientBubble(
                                          size: 220,
                                          color: Color(0x84FFE66A),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.28, 0.45),
                                        child: _AmbientBubble(
                                          size: 230,
                                          color: Color(0x805CD47A),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.28, 0.45),
                                      ...
                                      Align(
                                        ...
                                        child: _AmbientBubble(
                                          size: 120,
                                          color: Color(0x5EFF6D92),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.55, -0.08),
                                        child: _AmbientBubble(
                                          size: 120,
                                          color: Color(0x5D63B8FF),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(-0.50, 0.62),
                                        child: _AmbientBubble(
                                          size: 130,
                                          color: Color(0x5F61D189),
                                        ),
                                      ),
                                      Align(
                                        alignment: Alignment(0.50, 0.62),
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: AnimatedSwitcher(
                                    ...
                                    child: _terminalActiveScreen == 'functions'
                                        ? _buildTerminalFunctionsContent(cs: cs)
