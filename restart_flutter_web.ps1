...
function Stop-PortOwner {
  ...
  $conn = Get-ListeningConnection -Port $Port
  if ($null -eq $conn) {
  ...
  if ($null -ne $proc) {
    Write-Host "Stopping PID $ownerPid ($($proc.Name)) listening on port $Port"
    if ($proc.CommandLine) {
      Write-Host "Command: $($proc.CommandLine)"
    }
  } else {
    Write-Host "Stopping PID $ownerPid listening on port $Port"
  }
...
if (-not $SkipBackendRestart) {
  ...
  if (Test-Path -Path $startScript) {
    Write-Host 'Refreshing backend before Flutter launch...'
...
$webDevice = if ($NoBrowser) { 'web-server' } else { 'chrome' }
$paymentApiBaseUrl = "http://${BackendHost}:$BackendPort"

if (-not $NoBrowser -and [string]::IsNullOrWhiteSpace($ChromeUserDataDir)) {
  $ChromeUserDataDir = Join-Path $RepoRoot '.chrome-profile'
}
...
try {
  Write-Host "Starting Flutter on $webDevice at http://localhost:${WebPort}"
